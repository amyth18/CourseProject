...
def do_for_subset(iteration_number, number_of_rows, Num_rows, df_main):
    ...
    tfidf_text_vectorizer = TfidfVectorizer(ngram_range=(1, 3),
                                            stop_words=stopwords, min_df=1, max_df=0.8)
...
def data_source_type():
    if len(sys.argv) == 1:
        raise Exception("Please specify 'file' or 'db' as an argument.")
    elif sys.argv[1] != "file" and sys.argv[1] != "db":
        raise Exception("Invalid argument. Please specify 'file' or 'db' as an argument.")
    else:
        return sys.argv[1]


def save_results_to_db(mongodb, dframe):
    dbh = mongodb.get_db_handle()
    for idx in range(len(dframe)):
        row = dframe.loc[idx]
        topics = ""
        if not pd.isna(row['NMF_topic']) and row['NMF_topic'] != "":
            topics = [topic.strip() for topic in row['NMF_topic'].split('+')]
            logger.debug(topics)
            
        dbh.emails.update_one(
            {'_id': row['_id']},
            {
                '$set': {
                    'email_topics': topics
...
if __name__ == "__main__":
    ...
    if len(df_main) == 0:
        logger.info("There no emails to analyze. Please sync emails using UI.")
        sys.exit(0)

    # handle emails that done have text body. replace wirth subject.
    df_main.loc[df_main.text.isna(), 'text'] = df_main[df_main.text.isna()]['subject']
    df_main.loc[:, "NMF_topic"] = ""

    # df_main.loc[:, "TruncatedSVD_topic"] = ""
    # df_main.loc[:, "LDA_topic"] = ""
    logger.info("Starting topic discovery completed ...")
    ...
    if data_source_type() == "file":
        ...
        save_results_to_db(mdb, df_main)

    logger.info("Topic discovery completed!")
